...
The current Preview deployment, including production FAQ-type compatibility, completed successfully at `https://01m2727ryfv19vemq3kmb6ssz3-570f5129dce95a62cc8d.myshopify.dev`. Oxygen confirmed it was routable. Unauthenticated HTTP validation receives a `403` after redirect because this preview is protected; it is not a production-domain failure.

Following explicit launch approval, the same isolated storefront was deployed to its Oxygen `production` environment at `https://01m27gecmtj1qahgs4qncw3tnk-daa5878a4a43b7d93a09.myshopify.dev`. Oxygen confirmed it was routable. The `nenufar.mx` custom-domain mapping remains unchanged.
